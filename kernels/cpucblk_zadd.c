                   ...
                   const pastix_lr_t  *lowrank )
{
    ...
    for (; blokA < lblokA; blokA++) {
        ...
        params.C    = shift == PastixLCoef ? blokB->lLRblock : blokB->uLRblock;
                   ...
                   const pastix_lr_t  *lowrank )
{
    ...
    for (; blokA < lblokA; blokA++) {
        ...
        params.offx = blokA->frownum - blokB->frownum;
        params.C    = shift == PastixLCoef ? blokB->lLRblock : blokB->uLRblock;
        if ( shift == PastixLCoef ) {
            flops += core_zlradd( &params, blokA->lLRblock,
                                  PastixNoTrans, PASTIX_LRM3_ORTHOU );
        } else {
            flops += core_zlradd( &params, blokA->uLRblock,
                                  PastixNoTrans, PASTIX_LRM3_ORTHOU );
        }
    }
    return flops;
