                         ...
                         pastix_complex64_t *v1v2, pastix_int_t ldv )
{
    ...
    for (i=0; i<r2; i++, u2 += ldu, v2++) {
        ...
        if ( norm > (M * eps) ) {
            ...
            if ( i < (r2-1) ) {
                cblas_zswap( M, u2, 1, u1u2 + (r1+r2-1) * ldu, 1 );
                memset( u1u2 + (r1+r2-1) * ldu, 0,  M * sizeof(pastix_complex64_t) );

                cblas_zswap( N, v2, ldv, v1v2 + (r1+r2-1),     ldv );
                LAPACKE_zlaset_work( LAPACK_COL_MAJOR, 'A', 1, N,
                                     0., 0., v1v2 + (r1+r2-1), ldv );
                r2--;
                i--;
                u2-= ldu;
                v2--;
                   ...
                   pastix_complex64_t *v1v2, pastix_int_t ldv )
{
    ...
    for (i=r1; i<rank; i++, u2 += ldu, v2++) {
        ...
        if ( norm > M * eps ) {
            ...
            rank--; r2--;
            if ( i < rank ) {
                cblas_zswap( M, u2, 1, u1u2 + rank * ldu, 1 );
                memset( u1u2 + rank * ldu, 0,  M * sizeof(pastix_complex64_t) );

                cblas_zswap( N, v2, ldv, v1v2 + rank,     ldv );
                LAPACKE_zlaset_work( LAPACK_COL_MAJOR, 'A', 1, N,
                                     0., 0., v1v2 + rank, ldv );
                i--;
                u2-= ldu;
