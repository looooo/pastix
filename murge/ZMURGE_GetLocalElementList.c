...
#ifndef ZMURGE_GETLOCALELEMENTLIST_C
#  define ZMURGE_GETLOCALELEMENTLIST_C
#else
#  ifndef SUFFIX
#    error zmurge_getLocalElementList.c can be included only once, or use SUFFIX(name)
#  endif /* SUFFIX */
#endif
...
#  define ZMURGE_ElementListStorage  SUFFIX(ZMURGE_ElementListStorage)
#  define ZMURGE_ElementListStorageSize  \
  SUFFIX(ZMURGE_ElementListStorageSize)
#  define _element_  SUFFIX(_element_)
#  define _element_t SUFFIX(_element_t)
#endif

INTS * ZMURGE_ElementListStorage = NULL;
INTS   ZMURGE_ElementListStorageSize;

#ifdef FORCE_NOMPI
#  define MPI_SUCCESS 0
#  define MPI_INT  1
#  define MPI_LONG 2
#  define MPI_INTEGER4 3
#  define MPI_INTEGER8 4
#  define MPI_FLOAT    5
#  define MPI_DOUBLE   6
                              ...
                              MURGE_UserData_t * d)
{
  ...
  MURGE_FREE(ZMURGE_ElementListStorage);
  MURGE_MEMALLOC(idx, vertex_per_element, INTS);

  ierr = ZMURGE_GetCommRank(id, &rank); MURGE_CHKERR(ierr);
  ierr = ZMURGE_GetCommSize(id, &size); MURGE_CHKERR(ierr);
  ierr = ZMURGE_GetOptionINT(id, MURGE_IPARAM_BASEVAL, &baseval); MURGE_CHKERR(ierr);
  ierr = ZMURGE_GetComm(id, &comm); MURGE_CHKERR(ierr);
  ...
  switch(mode)
    ...
    case MURGE_DUPLICATE_ELEMENTS: {
      *localElementNbr = 0;
      ...
      ZMURGE_ElementListStorageSize = (*localElementNbr);
      MURGE_MEMALLOC(ZMURGE_ElementListStorage, (*localElementNbr), INTS);
      (*localElementNbr) = 0;
      for (i =0; i < globalElementNbr; i++)
        {
          GET_VERTICES(i, idx, d);
          for (k = 0; k < vertex_per_element; k++)
            if (owner[idx[k]-baseval] == rank) {
              ZMURGE_ElementListStorage[(*localElementNbr)++] = i;
              break;
            }
        }
      break;
    ...
    case MURGE_DISTRIBUTE_ELEMENTS: {
      {
        ...
        MURGE_START_TIMER;
        ZMURGE_ElementListStorageSize = eltsPerCPU_curr[rank];
        MURGE_MEMALLOC(ZMURGE_ElementListStorage,
...
INTS ZMURGE_GetLocalElementList(INTS id, INTS * element_list) {
  ...
  MURGE_START_TIMER;
  ierr = ZMURGE_GetOptionINT(id, MURGE_IPARAM_BASEVAL, &baseval); MURGE_CHKERR(ierr);
  if (ZMURGE_ElementListStorage == NULL)
    return MURGE_ERR_ORDER;
  for (i = 0; i < ZMURGE_ElementListStorageSize; i++)
    {
      element_list[i] = ZMURGE_ElementListStorage[i] + baseval;
    }
  MURGE_FREE(ZMURGE_ElementListStorage);
...
#  undef ZMURGE_GetLocalElementNbr
#  undef ZMURGE_GetLocalElementList
#  undef ZMURGE_ElementListStorage
#  undef ZMURGE_ElementListStorageSize
#  undef _element_
#  undef _element_t
#endif
