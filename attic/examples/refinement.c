/* File: refinement.c
   ...
   An example using refinement alone :
...
int main (int argc, char **argv)
{
  ...
  pastix_complex64_t *X0          = NULL; /* Initial guess of the solution.                            */
  ...
  /*******************************************/
  /* Call pastix from start to solve.        */
  /* RHS argument is used here to set X0.    */
  /*******************************************/
  perm = malloc(ncol*sizeof(pastix_int_t));
  invp = malloc(ncol*sizeof(pastix_int_t));

  X0 = malloc(ncol*sizeof(pastix_complex64_t));
  for (i = 0; i < ncol; i++)
    X0[i] = 0;

  PRINT_RHS("X0", rhs, ncol, rank, iparm[IPARM_VERBOSE]);
  pastix(&pastix_data, MPI_COMM_WORLD,
         ncol, colptr, rows, values,
         perm, invp, X0, nbrhs, iparm, dparm);
  free(X0);

  /*******************************************/
  /* Call PaStiX for refinement and clean.   */
  /*******************************************/
  iparm[IPARM_END_TASK] = API_TASK_CLEAN;
  PRINT_RHS("RHS", rhs, ncol, rank, iparm[IPARM_VERBOSE]);
  pastix(&pastix_data, MPI_COMM_WORLD,
         ncol, colptr, rows, values,
         perm, invp, rhs, nbrhs, iparm, dparm);

  PRINT_RHS("SOL", rhs, ncol, rank, iparm[IPARM_VERBOSE]);
  CHECK_SOL(rhs, rhssaved, ncol, rank);
