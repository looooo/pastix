 ...
 */
#ifdef TYPE_COMPLEX
static pastix_complex64_t fun   = 1.0+0.0*I;
#else
...
/*
 ...
 Floating point   0.0
 */
static pastix_complex64_t fzero = 0.0;
...
void z_dim_dgeam(char *transa,char *transb,
               pastix_int_t m,pastix_int_t n,pastix_complex64_t alpha,
...
/*         for (i = 0; i < size; i++) {                                    \ */
/*             if (tab[i] != tab[i]) {                                     \ */
/*                 fprintf(stdout, "%s:%s:%d NaN detected (%s)\n",         \ */
/*                         __FILE__, __FUNCTION__, __LINE__, #tab);        \ */
/*                 exit(1);                                                \ */
/*         }                                                               \ */
/*         }                                                               \ */
/*     } while(0) */
...
#  define SOPALIN_GEAM(i,j,m,n,x,a,u,b,v) do {  \
    TAB_CHECK_NAN(((pastix_complex64_t*)a), n*m);            \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), n*m);            \
...
#  define SOPALIN_GESM(i,j,m,n,x,a,u,b,v) do {  \
    TAB_CHECK_NAN(((pastix_complex64_t*)a), m*n);            \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), m*n);            \
...
#  define SOPALIN_COPY(n,x,u,y,v) {             \
    memcpy((void *)(y), (void *)(x),            \
           ((pastix_int_t)(n))*sizeof(pastix_complex64_t));           \
  }
#else /* BLAS_USE_COPY */
...
#  define SOPALIN_COPY(n,x,u,y,v) {             \
    ...
    TAB_CHECK_NAN(((pastix_complex64_t*)x), n);              \
    TAB_CHECK_NAN(((pastix_complex64_t*)y), n);              \
    COPY(&varin,                                \
         (BLAS_FLOAT*)(x), &variu,              \
         (BLAS_FLOAT*)(y), &variv);             \
    TAB_CHECK_NAN(((pastix_complex64_t*)y), n);              \
...
#  define SOPALIN_SCAL(n,a,x,u) {               \
    pastix_int_t i; pastix_complex64_t *pt,*p=(x);                    \
    for((pt)=(p+n);(p)<(pt);)                   \
      *((p)++)*= (a);                           \
  }
#else /* BLAS_USE_SCAL */
#  define SOPALIN_SCAL(n,a,x,u) {                                   \
    BLAS_INT varin     = (BLAS_INT)(n);                             \
    BLAS_INT variu     = (BLAS_INT)(u);                             \
    pastix_complex64_t    float_tmp = a;                                         \
...
#define SOPALIN_TRSM(s,p,t,d,m,n,x,a,u,b,v) {                     \
    ...
    BLAS_INT   variu     = (BLAS_INT)(u);                         \
    BLAS_INT   variv     = (BLAS_INT)(v);                         \
    pastix_complex64_t      float_tmp = x;                                     \
    BLAS_FLOAT varix     = *((BLAS_FLOAT*)((void*)&(float_tmp))); \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), m*n);                              \
    if ( *s == 'L' || *s == 'l' ) {                               \
      TAB_CHECK_NAN(((pastix_complex64_t*)a), m*m);                            \
    }                                                             \
    else {                                                        \
      TAB_CHECK_NAN(((pastix_complex64_t*)a), n*n);                            \
    }                                                             \
...
#define SOPALIN_TRSV(p,t,d,n,a,u,b,v) {                     \
    ...
    BLAS_INT variv = (BLAS_INT)(v);                         \
    TAB_CHECK_NAN(((pastix_complex64_t*)a), n*n);                        \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), n);                          \
                                                            \
    TRSV((p), (t), (d), &varin,                             \
         (BLAS_FLOAT*)(a), &variu,                          \
         (BLAS_FLOAT*)(b), &variv);                         \
...
#define SOPALIN_GEMM(i,j,m,n,k,x,a,u,b,v,y,c,w) {                     \
    ...
    TAB_CHECK_NAN(((pastix_complex64_t*)a), k*m);                                  \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), k*n);                                  \
    TAB_CHECK_NAN(((pastix_complex64_t*)c), m*n);                                  \
                                                                      \
...
#define SOPALIN_GEMV(i,m,n,x,a,u,b,v,y,c,w) {           \
    ...
    if (*i == 'N' || *i == 'n')   {                     \
      TAB_CHECK_NAN(((pastix_complex64_t*)b), n);                    \
      TAB_CHECK_NAN(((pastix_complex64_t*)c), m);                    \
    }                                                   \
    else {                                              \
      TAB_CHECK_NAN(((pastix_complex64_t*)b), m);                    \
      TAB_CHECK_NAN(((pastix_complex64_t*)c), n);                    \
    }                                                   \
...
#  define SOPALIN_SYR(i,n,x,a,u,b,v) {                                  \
    ...
    pastix_complex64_t      float_tmp = x;                                           \
    BLAS_FLOAT varix     = *((BLAS_FLOAT*) ((void*)&(float_tmp)));      \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), n*n);                                    \
    TAB_CHECK_NAN(((pastix_complex64_t*)a), n);                                      \
...
#define SOPALIN_SYRK(i,t,n,k,x,a,u,y,b,v) {            \
    ...
    TAB_CHECK_NAN(((pastix_complex64_t*)b), n*n);                   \
    if (*t == 'N' || *t == 'N') {                      \
      TAB_CHECK_NAN(((pastix_complex64_t*)a), k*n);                 \
    }                                                  \
    else {                                             \
      TAB_CHECK_NAN(((pastix_complex64_t*)a), n*k);                 \
    }                                                  \
...
#define SOPALIN_GER(m,n,x,a,u,b,v,c,w) {                                \
    ...
    pastix_complex64_t    float_tmp = x;                                             \
    BLAS_FLOAT varix   = *((BLAS_FLOAT*) ((void*)&(float_tmp)));        \
    TAB_CHECK_NAN(((pastix_complex64_t*)c), n*m);                                    \
    TAB_CHECK_NAN(((pastix_complex64_t*)a), n);                                      \
    TAB_CHECK_NAN(((pastix_complex64_t*)b), n);                                      \
