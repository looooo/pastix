...
#  define NAN (0.0 / 0.0)
#endif
#define OUT_INTL pastix_int_t
#define OUT_INTS pastix_int_t
...
INTS vcsc_add_node(variable_csc_t vcsc, INTS COL, INTS ROW, COEF * VALUE,
  ...
  COEF (*add_fct1)(COEF , COEF);

  switch(op) {
  case MURGE_ASSEMBLY_ADD:
    add_fct1 = &add_two_floats;
    break;
#ifndef TYPE_COMPLEX
        ...
        if (vcsc->dof) {
          ...
          for (ii = 0; ii < dof2; ii++) {
            if (isnan(vcsc->values[COL-1][i*dof2 + ii])) {
              ...
              vcsc->values[COL-1][i*dof2 + ii] = add_fct1(
...
INTS vcsc_add(variable_csc_t vcsc, INTS COL, INTS ROW, COEF VALUE,
  ...
  if (vcsc->dof < 2) {
    ...
    COEF (*add_fct2)(COEF , COEF);

    switch(op) {
    case MURGE_ASSEMBLY_ADD:
      add_fct2 = &add_two_floats;
      break;
#ifndef TYPE_COMPLEX
    case MURGE_ASSEMBLY_MAX:
      add_fct2 = &get_max;
        ...
        if (vcsc->rows[COL_NODE][i] == ROW_NODE+1) {
          ...
          if (isnan(vcsc->values[COL_NODE][idx])) {
            ...
            vcsc->values[COL_NODE][idx] =
              add_fct2(vcsc->values[COL_NODE][idx], VALUE);
          }
#if (defined MURGE_TRACE_COL && defined MURGE_TRACE_ROW)
          if (COL == MURGE_TRACE_COL &&
              ROW == MURGE_TRACE_ROW ) {
...
INTS vcsc_to_cscd(variable_csc_t   vcsc,
  ...
  COEF (*add_fct3)(COEF , COEF);

  switch(op) {
  case MURGE_ASSEMBLY_ADD:
    add_fct3 = &add_two_floats;
    break;
#ifndef TYPE_COMPLEX
  case MURGE_ASSEMBLY_MAX:
    add_fct3 = &get_max;
...
#else /* MURGE_SMART_L2G_COMPUTE */
    INTS first, last;
    *n = vcsc->n/procnbr;
    if (me < vcsc->n%procnbr)
      (*n)++;
    MURGE_REALLOC(*l2g_o, *n, OUT_INTS);
    first = (me*((long)(vcsc->n/procnbr))) + MIN(me, vcsc->n%procnbr) + 1;
    for (iter = 0; iter < *n; iter++) {
      (*l2g_o)[iter] = first+iter;
    }
#endif /* MURGE_SMART_L2G_COMPUTE */
    CLOCK_PRINT("computing l2g");
    {
      pastix_int_t vcsc_n = (pastix_int_t)vcsc->n;
      z_cscd_build_g2l(*n,
                       *l2g_o,
                       comm,
                       &vcsc_n,
            ...
            for (iterdof = 0; iterdof < dof2; iterdof++) {
              if (!isnan(vcsc->values[MYCOL-1][dof2*iter2+iterdof])) {
                ...
                (*values_o)[dof2*iter3+iterdof] =
                  add_fct3((*values_o)[dof2*iter3+iterdof],
