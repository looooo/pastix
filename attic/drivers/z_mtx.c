...
void z_ijvRead(char *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType, pastix_int_t flagsort)
{
  ...
  *RhsType = (char *) malloc(1*sizeof(char));
  ...
  /* Lire Entete */
  z_ijvReadHeader(infile, Nrow, Ncol, Nnzero, *Type);
  ...
  if ((tempcol==NULL) || ((*row) == NULL) || ((*val) == NULL))
    {
      fprintf(stderr, "z_mtxRead : Not enough memory\n");
	...
	if ( (tempcol[iter2+1] < tempcol[iter2]) )
	  {
	    ...
	    pastix_complex64_t swapv = (*val)[iter2+1];
...
void z_rsaRead(char const *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType)
{
  ...
  *val   =(pastix_complex64_t*)malloc((*Nnzero)  *sizeof(pastix_complex64_t));ASSERT(*val  !=NULL,MOD_SI);
...
void z_HBRead(char const *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, 
	    pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType)
{
  int i;
  int nrhs;
  int tmpNrow,tmpNcol,tmpNnzero;
...
/*   printf("RSA: Nrow=%ld Ncol=%ld Nnzero=%ld\n",(long)*Nrow,(long)*Ncol,(long)*Nnzero); */
#ifdef TYPE_COMPLEX
  fprintf(stderr,"Warning: z_HBRead is a real matrix driver\n");
  exit(EXIT_FAILURE);
#endif
...
void z_dimsym(pastix_int_t n, pastix_int_t **ia, pastix_int_t **ja)
{
  pastix_int_t *add;
...
void z_cccRead(char const * filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType)
{
  ...
  *RhsType = (char *) malloc(4*sizeof(char));
  ...
  z_cccReadHeader(infile, Nrow, Ncol, Nnzero, *Type);
  fclose(infile);
  ...
  if (((*col) == NULL) || ((*row) == NULL) || ((*val) == NULL))
    fprintf(stderr, "z_cccRead : Not enough memory for \n");
...
#ifdef TYPE_COMPLEX
	  (*val)[i] = pastix_complex64_t(temp1,temp2);
#else
	  (*val)[i] = temp1;
#endif
#else
	  (*val)[i] = temp1;
	  (*val)[i] += ((pastix_complex64_t)temp2)*I;
#endif
	  i++;
	}
#else
      (*val)[iter] = temp1;
      (*val)[iter] += ((pastix_complex64_t)temp2)*I;
#endif
    }
  fclose(infile1);
  fclose(infile2);
...
void z_chbReadHeader(FILE *infile, char *Type, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t *Nrhs, char *Ptrfmt, char *Indfmt, char *Valfmt, char *Rhsfmt, pastix_int_t *Ptrcrd, pastix_int_t *Indcrd, pastix_int_t *Valcrd, pastix_int_t *Rhscrd,  char *RhsType)
{
  char line[BUFSIZ];
  long totcrd;
  long neltvl;
...
void z_chbRead(char const *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row,
	     pastix_complex64_t **val, char **Type, char **RhsType, pastix_complex64_t **rhs)
{
  FILE *infile;
  pastix_int_t Nrhs;
  char Ptrfmt[17];
  ...
  *RhsType = (char *) malloc(4*sizeof(char));
  ...
  z_chbReadHeader(infile, *Type, Nrow, Ncol, Nnzero, &Nrhs, Ptrfmt, Indfmt, Valfmt, Rhsfmt, &Ptrcrd, &Indcrd, &Valcrd, &Rhscrd, *RhsType);
  ...
  if (element == NULL)
    {
      fprintf(stderr, "z_chbRead : Not enough memory for element\n");
      EXIT(MOD_SI,OUTOFMEMORY_ERR);
    }
  element[Ptrwidth] = '\0';
  ...
  if ((*col) == NULL)
    {
      fprintf(stderr, "z_chbRead : Not enough memory for *col\n");
      EXIT(MOD_SI,OUTOFMEMORY_ERR);
    }
  ...
  if ((*row) == NULL)
    {
      fprintf(stderr, "z_chbRead : Not enough memory for *row\n");
      EXIT(MOD_SI,OUTOFMEMORY_ERR);
    }
  ...
  if ((*val) == NULL)
    {
      fprintf(stderr, "z_chbRead : Not enough memory for *val\n");
      EXIT(MOD_SI,OUTOFMEMORY_ERR);
    }
  ...
  for (iter=0; iter<Valcrd; iter++)
      ...
      for (item=0; item<Valperline; item++)
	{
	  ...
	  if (MTX_ISCOM(*Type))
		...
		{
#if (defined X_ARCHalpha_compaq_osf1)
		  (*val)[(count-1)/2] += pastix_complex64_t(0.0, atof(element));
#else
  ...
  if (Rhscrd != 0)
      ...
      if (element == NULL)
	{
      ...
      if (MTX_ISRHX(*RhsType))
	(*rhs) = (pastix_complex64_t *) malloc(2*(*Ncol)*sizeof(pastix_complex64_t));
      else
	(*rhs) = (pastix_complex64_t *) malloc((*Ncol)*sizeof(pastix_complex64_t));
      if ((*rhs) == NULL)
	{
	  fprintf(stderr, "z_chbRead : Not enough memory for *rhs\n");
          EXIT(MOD_SI,OUTOFMEMORY_ERR);
	}
	  ...
	  for (item=0; item<Rhsperline; item++)
	      ...
	      if (MTX_ISCOM(*Type))
		{
		  if (count % 2)
		    {
#if (defined X_ARCHalpha_compaq_osf1)
		      (*rhs)[(count-1)/2] += pastix_complex64_t(0.0, atof(element));
#else
      ...
      if (MTX_ISRHX(*RhsType))
	{
	  ...
	  if (rhs2 == NULL)
	    {
	      fprintf(stderr, "z_chbRead : Not enough memory for *rhs2\n");
	      EXIT(MOD_SI,OUTOFMEMORY_ERR);
	    }
	  ...
	  for (iter=0; iter<nbline; iter++)
	      ...
	      for (item=0; item<Rhsperline; item++)
		{
		  ...
		  if (MTX_ISCOM(*Type))
			...
			{
#if (defined X_ARCHalpha_compaq_osf1)
			  rhs2[(count-1)/2] += pastix_complex64_t(0.0, atof(element));
#else
...
void z_peerRead(char const *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType, pastix_complex64_t **rhs)
{
  ...
  *Ncol = *Nrow;
  ...
  (*col) = (pastix_int_t *) malloc((*Nrow+1)*sizeof(pastix_int_t));
  if ((*col) == NULL)
    fprintf(stderr, "z_peerRead : Not enough memory for *col\n");
  (*row) = (pastix_int_t *) malloc(*Nnzero*sizeof(pastix_int_t));
  if ((*row) == NULL)
    fprintf(stderr, "z_peerRead : Not enough memory for *row\n");
  (*val) = (pastix_complex64_t *) malloc(*Nnzero*sizeof(pastix_complex64_t));
  if ((*val) == NULL)
    fprintf(stderr, "z_peerRead : Not enough memory for *val\n");
  (*rhs) = (pastix_complex64_t *) malloc(*Nrow*sizeof(pastix_complex64_t));
  if ((*rhs) == NULL)
    fprintf(stderr, "z_peerRead : Not enough memory for *rhs\n");
  ...
  for (iterfile=0; iterfile<filenamenumber; iterfile++)
      ...
      for (iterelt=0; iterelt<nzlocal+1-nbreltperline; iterelt++)
	{
	  ...
	  (*val)[iterelt+nzglobal]   = (pastix_complex64_t)tempfloat1;
	  (*val)[iterelt+nzglobal+1] = (pastix_complex64_t)tempfloat2;
	  (*val)[iterelt+nzglobal+2] = (pastix_complex64_t)tempfloat3;
	  (*val)[iterelt+nzglobal+3] = (pastix_complex64_t)tempfloat4;
	  iterelt+=3;
	}
      switch (nzlocal-iterelt)
	{
      ...
      for (iterelt=0; iterelt<rowlocal+1-nbreltperline; iterelt++)
	{
	  fgets(line,BUFSIZ,infile);
	  sscanf(line,"%Lf %Lf %Lf %Lf",&tempfloat1,&tempfloat2,&tempfloat3,&tempfloat4);
	  (*rhs)[iterelt+rowglobal]   = (pastix_complex64_t)tempfloat1;
	  (*rhs)[iterelt+rowglobal+1] = (pastix_complex64_t)tempfloat2;
	  (*rhs)[iterelt+rowglobal+2] = (pastix_complex64_t)tempfloat3;
	  (*rhs)[iterelt+rowglobal+3] = (pastix_complex64_t)tempfloat4;
	  iterelt+=3;
	}
...
void peerRead2(char const *filename, pastix_int_t *Nrow, pastix_int_t *Ncol, pastix_int_t *Nnzero, pastix_int_t **col, pastix_int_t **row, pastix_complex64_t **val, char **Type, char **RhsType, pastix_complex64_t **rhs)
{
  FILE *infile;
  pastix_int_t iterfile;
  char line[BUFSIZ];
...
void z_mumpsRead(char const *dirname, 
	      pastix_int_t *Ncol, 
	      pastix_int_t *Nrow, 
	      pastix_int_t *Nnzero, 
	      pastix_int_t **col, 
  ...
  *RhsType = (char *) malloc(1*sizeof(char));
  ...
  z_mumpsReadHeader(headerFile,Nrow,Ncol,Nnzero,*Type);
  fclose (headerFile);
  ...
  if ((tempcol==NULL) || (temprow == NULL) || (tempval == NULL))
    {
      fprintf(stderr, "z_mumpsRead : Not enough memory (Nnzero %ld)\n",(long)*Nnzero);
      EXIT(MOD_SI,OUTOFMEMORY_ERR);
    }
