 ...
 * Parameters:
 *   a - A coefficient.
 *   c - Vector of 16 <cuDoubleComplex>.
 *   b - Vector of 16 <cuDoubleComplex>.
 */
static __device__ void
axpy(cuDoubleComplex a, cuDoubleComplex *b, cuDoubleComplex *c) {
...
#undef LROWNUM
  ...
  cuDoubleComplex Dp[4]={D[0], D[t2], D[t3], D[t4]};
  ...
  if(k>7)
    do {
      cuDoubleComplex Ab[4] = {MULT(Ap[0],Dp[0]), MULT(Ap[1],Dp[1]), MULT(Ap[2],Dp[2]), MULT(Ap[3],Dp[3])};
						...
						const int * fblocktab)
{
  ...
  dim3 grid(m/64+(m%64!=0),n/16+(n%16!=0));
#endif
  sparse_zgemdm_kernel_N_T_64_16_4_16_4
