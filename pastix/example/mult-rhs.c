...
int main (int argc, char **argv)
{
  ...
  for(i=0; i<nbrhs; i++) {
    PRINT_RHS("RHS", (&(rhs[i*ncol])), ncol, mpid, iparm[IPARM_VERBOSE]);
  }
  pastix(&pastix_data, MPI_COMM_WORLD,
         ncol, colptr, rows, values,
  ...
  for(i=0; i<nbrhs; i++) {
    PRINT_RHS("SOL (Factorization and solve)",
	      (&(rhs[i*ncol])), ncol, mpid, iparm[IPARM_VERBOSE]);
  ...
  for(i=0; i<nbrhs; i++) {
    ...
    pastix(&pastix_data, MPI_COMM_WORLD,
           ncol, colptr, rows, values,
           perm, invp, &(rhs[i*ncol]), 1, iparm, dparm);
    PRINT_RHS("SOL (solve and refinement)", (&(rhs[i*ncol])), ncol, mpid, iparm[IPARM_VERBOSE]);
    CHECK_SOL((&(rhs[i*ncol])), (&(rhssaved[i*ncol])), ncol, mpid);
  }
