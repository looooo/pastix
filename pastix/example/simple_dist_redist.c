...
int main (int argc, char **argv)
{
  ...
  /* Here the user should do something smarter than
   * calling cscd_redispatch().
   * eg: create a local element list using the local node list
   *     and only build what is localy needed
   * Calling cscd_redispach() is already done inside PaStiX if user's
   * distribution is not PaStiX's one.
   */
