...
#pragma omp parallel shared(nb_threads)
  ...
  MURGE_AssemblyBegin(id, n, nnzeros, MURGE_ASSEMBLY_OVW, MURGE_ASSEMBLY_OVW,
                      MURGE_ASSEMBLY_FOOL, MURGE_BOOLEAN_FALSE);

#ifdef _OPENMP
#pragma omp parallel default(none) private(m, i, ierr, k, matElem)      \
  shared(dof, localnodenbr, n, xmin, xmax, nodelist, id)
#endif /* _OPENMP */
  {
...
#pragma omp for
    ...
    for (m = 0; m < localnodenbr; m++) {
      ...
      if ( i == 1 || i == n ) {
        ...
        MURGE_AssemblySetNodeValues(id, i, i, matElem);
      } else {
        for (k = -1; k <= 1; k++) {
          GetCoef(matElem,i+k,i,xmin,xmax,n, dof);
          MURGE_AssemblySetNodeValues(id, i, i+k, matElem);
        }
      }
    }
    free(matElem);
