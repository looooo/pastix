 ...
 *   void
 */
void updo_down_recv ( z_Sopalin_Data_t *sopalin_data,
                      void           *updo_buffer,
                      MPI_Status      status,
                      pastix_int_t             me )
{
  z_SolverMatrix  *datacode    = sopalin_data->datacode;
  pastix_complex64_t         *gb, *gc;
...
#  endif /* TRACE_SOPALIN */
  ...
  gb   = ((pastix_complex64_t*)(updo_buffer)) + UPDOWN_SIZETAB*sizeof(pastix_int_t)/sizeof(pastix_complex64_t);
 ...
 *   void
 */
void updo_down_send ( z_Sopalin_Data_t *sopalin_data, pastix_int_t me, pastix_int_t i, pastix_int_t j )
{
  z_SolverMatrix  *datacode    = sopalin_data->datacode;
  z_Thread_Data_t *thread_data = sopalin_data->thread_data[me];
...
#  ifdef NO_MPI_TYPE
  ...
  tabtype[1] = sizeof(pastix_complex64_t);
  taboffs[0] = infotab;
...
#    endif /* PASTIX_UPDO_ISEND */
  COMM_CLOCK_STOP;
#    ifdef OOC_FTGT
  z_ooc_reset_ftgt(sopalin_data, SOLV_FTGTIND(j), me);
#    else /* not OOC_FTGT */
                                ...
                                pastix_int_t      i )
{
  ...
  print_debug(DBG_SOPALIN_UP, "%d : z_Task %4d Wait %4d\n",
 ...
 */
#  ifndef FORCE_NOMPI
void updo_up_recv ( z_Sopalin_Data_t *sopalin_data,
                    void           *updo_buffer,
                    MPI_Status      status,
                    pastix_int_t             me)
{
  ...
  pastix_int_t           *infotab;
  pastix_complex64_t         *gb, *lgb, *lgrhs;
...
#    endif /* TRACE_SOPALIN */
  ...
  gb      = ((pastix_complex64_t*)(updo_buffer)) +
    UPDOWN_SIZETAB*sizeof(pastix_int_t)/sizeof(pastix_complex64_t);
 ...
 */
#ifndef FORCE_NOMPI
void updo_up_send ( z_Sopalin_Data_t *sopalin_data, pastix_int_t me, pastix_int_t i, pastix_int_t j)
{
  z_SolverMatrix  *datacode    = sopalin_data->datacode;
  z_Thread_Data_t *thread_data = sopalin_data->thread_data[me];
#  if (defined PASTIX_UPDO_ISEND) || (defined NO_MPI_TYPE)
...
#  ifdef   NO_MPI_TYPE
  ...
  for (iter=1; iter<(UPDOWN_SM2XNBR+1); iter++)
    {
      tabsize[iter] = size;
      tabtype[iter] = sizeof(pastix_complex64_t);
...
/******************************************************************************/
#ifndef FORCE_NOMPI
void* z_updo_thread_comm ( void *arg )
{
  ...
  if (THREAD_COMM_ON)
    ...
    {
      errorPrint("z_updo_thread_comm should not be called in THREAD_COMM mode.");
      return (void *)1;
    }
}
#endif /* FORCE_NOMPI */
