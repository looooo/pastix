...
void* z_pivotstatique_smp ( void *arg )
{
  ...
  sopar->dparm[DPARM_RAFF_TIME] =  clockVal((raff_clk));
...
void* API_CALL(z_gmres_smp)(void *arg)
{
  ...
  MALLOC_INTERN(gmrestemp,  UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(gmresb,     UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(gmresc,     gmresim,        pastix_complex64_t);
  MALLOC_INTERN(gmress,     gmresim,        pastix_complex64_t);
  MALLOC_INTERN(gmresrs,    gmresim+1,      pastix_complex64_t);
  MALLOC_INTERN(gmresvv,    gmresim+1,      pastix_complex64_t*);
  MALLOC_INTERN(gmreshh,    gmresim,        pastix_complex64_t*);
  MALLOC_INTERN(gmresw,     gmresim,        pastix_complex64_t*);
  ...
  for(i=0; i<(gmresim+1); i++)
    {
      MALLOC_INTERN(gmresvv[i], UPDOWN_SM2XSZE, pastix_complex64_t);
    }

  for (i=0; i<gmresim; i++)
    {
      MALLOC_INTERN(gmreshh[i], gmresim+1,      pastix_complex64_t);
      MALLOC_INTERN(gmresw[i],  UPDOWN_SM2XSZE, pastix_complex64_t);
    }
  ...
  MONOTHREAD_END;
  SYNCHRO_THREAD;
  gmrestemp  = (pastix_complex64_t *) sopalin_data->ptr_raff[0];
  gmresb     = (pastix_complex64_t *) sopalin_data->ptr_raff[1];
  gmresvv    = (pastix_complex64_t **)sopalin_data->ptr_raff[2];
  gmreshh    = (pastix_complex64_t **)sopalin_data->ptr_raff[3];
  gmresc     = (pastix_complex64_t *) sopalin_data->ptr_raff[4];
  gmress     = (pastix_complex64_t *) sopalin_data->ptr_raff[5];
  gmresrs    = (pastix_complex64_t *) sopalin_data->ptr_raff[6];
  gmresw     = (pastix_complex64_t **)sopalin_data->ptr_raff[7];
  ...
  while (sopalin_data->gmresout_flag)
    {
      ...
      /* wk2 = A*x */
      z_CscAx(sopalin_data, me, sopalin_data->sopar->cscmtx,
      ...
      /* ro = vv[0].vv[0] */
      z_CscGmresBeta(sopalin_data, me, gmresvv[0], gmresvv[0],
      ...
      SYNCHRO_THREAD;
      if ((double)ABS_FLOAT((pastix_complex64_t)sopalin_data->gmresro) <=
          sopar->epsilonraff)
        {
      ...
      gmrest = (pastix_complex64_t)(1.0/sopalin_data->gmresro);

      MONOTHREAD_BEGIN;

      SOPALIN_SCAL(UPDOWN_SM2XSZE, gmrest, gmresvv[0], gmresincx);

      gmresrs[0] = (pastix_complex64_t)sopalin_data->gmresro;
      ...
      while(sopalin_data->gmresin_flag)
        {
          ...
          for (j=0; j<=i; j++)
            {
              /* vv[j]*vv[i1] */
              z_CscGmresBeta(sopalin_data, me, gmresvv[gmresi1], gmresvv[j],
                           UPDOWN_SM2XSZE, 1, &beta, pastix_comm);
          ...
          z_CscGmresBeta(sopalin_data, me, gmresvv[gmresi1],gmresvv[gmresi1],
                       UPDOWN_SM2XSZE, 1, &beta, pastix_comm);
...
#ifdef TYPE_COMPLEX
          gmrest = (pastix_complex64_t)csqrt(ABS_FLOAT(gmreshh[i][i]*gmreshh[i][i])+
          ...
          if (ABS_FLOAT(gmrest) <= sopar->epsilonraff)
            gmrest = (pastix_complex64_t)sopar->epsilonraff;
...
void* API_CALL(z_grad_smp)(void *arg)
{
  ...
  MALLOC_INTERN(gradb, UPDOWN_SM2XNBR*UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(gradr, UPDOWN_SM2XNBR*UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(gradp, UPDOWN_SM2XNBR*UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(gradz, UPDOWN_SM2XNBR*UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(grad2, UPDOWN_SM2XNBR*UPDOWN_SM2XSZE, pastix_complex64_t);
  MALLOC_INTERN(alpha, UPDOWN_SM2XNBR               , pastix_complex64_t);
  MALLOC_INTERN(beta,  UPDOWN_SM2XNBR               , pastix_complex64_t);
  ...
  MONOTHREAD_END;
  SYNCHRO_THREAD;
  gradb = (pastix_complex64_t *)sopalin_data->ptr_raff[0];
  gradp = (pastix_complex64_t *)sopalin_data->ptr_raff[1];
  gradr = (pastix_complex64_t *)sopalin_data->ptr_raff[2];
  gradz = (pastix_complex64_t *)sopalin_data->ptr_raff[3];
  grad2 = (pastix_complex64_t *)sopalin_data->ptr_raff[4];
  ...
  /* r=b-ax */
  z_CscbMAx(sopalin_data, me, gradr, gradb, sopalin_data->sopar->cscmtx,
          &(datacode->updovct), datacode, pastix_comm,
          sopar->iparm[IPARM_TRANSPOSE_SOLVE]);

  tmp = z_CscNormErr( sopalin_data, me, gradr, gradb, UPDOWN_SM2XSZE,
  ...
  print_debug(DBG_SOPALIN_RAFF, "z_raff.c:%d: %ld stop %g\n",__LINE__, (long)me, sopalin_data->stop);
  ...
  while ((sopalin_data->stop > sopar->epsilonraff) && (sopalin_data->count_iter < sopar->itermax))
    {
...
#endif
      /* grad2 = Ap */
      z_CscAx( sopalin_data, me, sopalin_data->sopar->cscmtx, gradp, grad2,
             datacode, &(datacode->updovct), pastix_comm,
             sopar->iparm[IPARM_TRANSPOSE_SOLVE]);
      /* alpha = <r,z>/<Ap,p> */

      z_CscGradBeta(sopalin_data, me, gradr, gradz,
                  UPDOWN_SM2XSZE, UPDOWN_SM2XNBR, beta, pastix_comm);
      z_CscGradBeta(sopalin_data, me, grad2, gradp,
      ...
      */
...
#ifdef MULT_SMX_RAFF
      {
        ...
        for(itersmx=0; itersmx<UPDOWN_SM2XNBR;itersmx++)
          {
            tmp_flt = (pastix_complex64_t) alpha[itersmx];
...
#else
      tmp_flt = (pastix_complex64_t) alpha[0];
...
#else
      tmp_flt = (pastix_complex64_t)beta[0];
      ...
      tmp = z_CscNormErr(sopalin_data, me, gradr, gradb, UPDOWN_SM2XSZE, UPDOWN_SM2XNBR, pastix_comm);
      ...
      MONOTHREAD_BEGIN;
      sopalin_data->stop = tmp;
      print_debug(DBG_RAFF_GRAD, "z_raff.c:%d: %ld stop %g\n",__LINE__, (long)me, sopalin_data->stop);
...
/*
  Function: API_CALL(z_gmres_thread)

  Launch sopaparam->nbthrdcomm threads which will compute
  <API_CALL(z_gmres_smp)>.

  Parameters:
  datacode  - PaStiX <z_SolverMatrix> structure.
  sopaparam - <z_SopalinParam> parameters structure.
*/
void API_CALL(z_gmres_thread)(z_SolverMatrix *datacode,
                            z_SopalinParam *sopaparam)
{
  z_Sopalin_Data_t *sopalin_data = NULL;
  BackupSolve_t b;
...
/*
  Function: API_CALL(z_grad_thread)
  ...
  sopaparam - <z_SopalinParam> parameters structure.
*/
void API_CALL(z_grad_thread)(z_SolverMatrix *datacode,
                           z_SopalinParam *sopaparam)
{
  z_Sopalin_Data_t *sopalin_data = NULL;
  BackupSolve_t b;
