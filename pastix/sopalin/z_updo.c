...
#include "z_updo_sendrecv.h"

#define z_updo_thread API_CALL(z_updo_thread)
/* Lancement de updo seul */
void z_updo_thread ( z_SolverMatrix * datacode, z_SopalinParam * sopar);

/*********************************/
/*
 * Function: API_CALL(z_updo_thread)
 *
 * Launch threads for solving step.
 *
 * Parameters:
 *   datacode  - z_SolverMatrix structure (common data)
 *   sopaparam - sopalin parameters.
 *
 * Returns:
 *   void
 */
void z_updo_thread ( z_SolverMatrix *datacode, z_SopalinParam *sopaparam )
{
  ...
  /*     z_sopalin_init(sopalin_data, datacode, sopaparam); */
...
void* z_up_down_smp ( void *arg )
{
...
#else
  int               itersmx;
  pastix_complex64_t            *lgb;
  pastix_complex64_t            *lgrhs;
#endif
  ...
  Queue             cblreadyqueue;
#endif
  pastix_complex64_t *b = NULL;
  MONOTHREAD_BEGIN;

  if (sopalin_data->sopar->iparm[IPARM_VERBOSE] > API_VERBOSE_CHATTERBOX)
    fprintf(stdout,  "%d:%d z_up_down_smp\n", (int)SOLV_PROCNUM, (int)me);

  z_ooc_set_step(sopalin_data, OOCSTEP_DOWN);
  ...
  if (sopalin_data->sopar->iparm[IPARM_END_TASK] == API_TASK_SOLVE &&
    ...
    MALLOC_INTERN(b, UPDOWN_SM2XSZE, pastix_complex64_t);
      ...
      for (itercblk=0; itercblk<SYMB_CBLKNBR; itercblk++)
        {
          ...
          for (col = 0;
               ...
               col++)
            {
              if (stride-(col+1) > 0)
                {
                  pastix_complex64_t d  = ((pastix_complex64_t)1.)/SOLV_UCOEFTAB(itercblk)[
                    SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                    col*(stride+1)];
                  pastix_complex64_t *v = &(SOLV_UCOEFTAB(itercblk)[
                                 SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                                 col*(stride+1)+1]);
      ...
      for (itercblk=0; itercblk<SYMB_CBLKNBR; itercblk++)
        {
          ...
          for (col = 0;
               ...
               col++)
            {
              if (stride-(col+1) > 0)
                {
                  pastix_complex64_t d  = SOLV_COEFTAB(itercblk)[
                    SOLV_COEFIND(SYMB_BLOKNUM(itercblk))+
                    col*(stride+1)];
                  pastix_complex64_t *v = &(SOLV_COEFTAB(itercblk)[
                                 SOLV_COEFIND(SYMB_BLOKNUM(itercblk))+
                                 col*(stride+1)+1]);
                  SOPALIN_SCAL(stride-(col+1), d, v, iun);
                }
...
#  endif /* PASTIX_DUMP_SOLV */
  ...
  z_sopalin_init_smp(sopalin_data, me, 0, init);
  ...
  for (i=0;i<SYMB_CBLKNBR;i++)
    ...
    {
      UPDOWN_SM2XTAB[UPDOWN_SM2XIND(i)+j] += 3.0*(pastix_complex64_t)SYMB_NODENBR;
    }
#endif /* UPDO_DEADCODE */
...
#  endif
  ...
  while(1){
    ...
    ii = API_CALL(z_sopalin_dynsched_getNexTask)( sopalin_data, datacode, thread_data,
...
#  endif /* FORCE_NOMPI */
    ...
    /*********************************/
    {
      ...
      z_ooc_wait_for_cblk(sopalin_data, i, me);
...
#  endif /* CHOL_SOPALIN */
      ...
      z_ooc_save_coef(sopalin_data, i, i, me);
...
#  endif /* CHOL_SOPALIN */
  ...
  /* Init Up monothread */
  SYNCHRO_THREAD; /* INUTILE sauf peut-etre pour z_ooc_set_step */

  MONOTHREAD_BEGIN;
  z_ooc_set_step(sopalin_data, OOCSTEP_UP);
  trace_begin_task(sopalin_data->tracefile,
                   SOPALIN_CLOCK_TRACE, SOLV_PROCNUM, me, 0,
                   STATE_L0_UPDOINIT, 0);
...
#  elif (defined SMP_SOPALIN)
  ...
  {
    i = queueGet(&cblreadyqueue);
    print_debug(DBG_SOPALIN_UP, "%d : z_Task %4d\n", (int)me, (int)i);
...
#  endif
      ...
      for (j=UPDOWN_BROWPROCNBR(i)-1;j>=0;j--)
      {
        ...
        if (sopalin_data->sopar->iparm[IPARM_SCHUR] == API_YES &&
            UPDOWN_BROWCBLKTAB(i)[j] == UPDOWN_GCBLKNBR-1)
          continue;
...
#  ifndef STORAGE /* On calcul au fur et a mesure les contributions */
        ...
        else
        {
               ...
               count++)
          {
            ...
            z_ooc_wait_for_cblk(sopalin_data, kk, me);
...
#  endif /* PASTIX_UPDO_ISEND */
  ...
  sopar->dparm[DPARM_SOLV_TIME] = SOPALIN_CLOCK_GET;
      ...
      for (itercblk=0; itercblk<SYMB_CBLKNBR; itercblk++)
        {
          ...
          for (col = 0;
               ...
               col++)
            {
              if (stride-(col+1) > 0)
                {
                  pastix_complex64_t d = SOLV_UCOEFTAB(itercblk)[SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                                                    col*(stride+1)];

                  pastix_complex64_t *v = &(SOLV_UCOEFTAB(itercblk)[SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                                                       col*(stride+1)+1]);
                  SOPALIN_SCAL(stride-(col+1), d, v, iun);
                }
            }
      ...
      for (itercblk=0; itercblk<SYMB_CBLKNBR; itercblk++)
        {
          ...
          for (col = 0;
               ...
               col++)
            {
              if (stride-(col+1) > 0)
                {
                  pastix_complex64_t d = ((pastix_complex64_t)1.)/SOLV_COEFTAB(itercblk)[SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                                                               col*(stride+1)];
                  pastix_complex64_t *v = &(SOLV_COEFTAB(itercblk)[SOLV_COEFIND(SYMB_BLOKNUM(itercblk)) +
                                                      col*(stride+1)+1]);
                  SOPALIN_SCAL(stride-(col+1), d, v, iun);
                }
            }
...
#  endif
  ...
  if (sopalin_data->sopar->iparm[IPARM_END_TASK] == API_TASK_SOLVE &&
    ...
    b = (pastix_complex64_t *)sopalin_data->ptr_raff[2];
    ...
    /* |A||x| + |b| */
    z_CscAxPb( sopalin_data, me, s, b, sopalin_data->sopar->cscmtx,
             &(datacode->updovct), datacode, PASTIX_COMM,
             sopar->iparm[IPARM_TRANSPOSE_SOLVE]);
    ...
    prec = z_CscNormErr(sopalin_data,
                      me,
                      r,
                      b,
                      UPDOWN_SM2XSZE,
