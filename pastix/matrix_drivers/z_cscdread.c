...
/*   fprintf(stderr, "[P%d] nzlocal %ld\n", myrank, (long) edgeloc); */
  ...
  *colptr      = (pastix_int_t *)   malloc((vertloc+1) * sizeof(pastix_int_t));
  if (   (*colptr)   == NULL)
    fprintf(stderr, "[P%d] z_cscdRead : Not enough memory for *colptr\n",myrank);
  *loc2glb = (pastix_int_t *)   malloc(  vertloc   * sizeof(pastix_int_t));
  if ( (*loc2glb) == NULL)
    fprintf(stderr, "[P%d] z_cscdRead : Not enough memory for *loc2glb\n",myrank);
  *row      = (pastix_int_t *)   malloc(  edgeloc   * sizeof(pastix_int_t));
  if (  (*row)   == NULL)
    fprintf(stderr, "[P%d] z_cscdRead : Not enough memory for *row\n",myrank);
  *avals   = (pastix_complex64_t *) malloc(  edgeloc   * sizeof(pastix_complex64_t));
  if (  (*avals) == NULL)
    fprintf(stderr, "[P%d] z_cscdRead : Not enough memory for *aval\n",myrank);
  memset(*avals, 0, (edgeloc)*sizeof(pastix_complex64_t));
  *rhs     = (pastix_complex64_t *) malloc(  vertloc   * sizeof(pastix_complex64_t));
  if (   (*rhs)  == NULL)
    fprintf(stderr, "[P%d] z_cscdRead : Not enough memory for *rhs\n",myrank);
  memset(*rhs, 0, (vertloc)*sizeof(pastix_complex64_t));
  ...
  for (iterelt=0; iterelt<edgeloc+1-nbreltperline; iterelt+=4)
    {
      ...
      (*avals)[iterelt]   = (pastix_complex64_t)tempfloat1;
      (*avals)[iterelt+1] = (pastix_complex64_t)tempfloat2;
      (*avals)[iterelt+2] = (pastix_complex64_t)tempfloat3;
      (*avals)[iterelt+3] = (pastix_complex64_t)tempfloat4;
    }
  ...
  for (iterelt=0; iterelt<vertloc+1-nbreltperline; iterelt+=4)
    {
      FGETS(line,BUFSIZ,infile);
      sscanf(line,"%lf %lf %lf %lf",&tempfloat1,&tempfloat2,&tempfloat3,&tempfloat4);
      (*rhs)[iterelt]   = (pastix_complex64_t)tempfloat1;
      (*rhs)[iterelt+1] = (pastix_complex64_t)tempfloat2;
      (*rhs)[iterelt+2] = (pastix_complex64_t)tempfloat3;
      (*rhs)[iterelt+3] = (pastix_complex64_t)tempfloat4;
    }
