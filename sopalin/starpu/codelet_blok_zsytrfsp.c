...
/**
 * @brief Main structure for all tasks of blok_zsytrfsp type
 */
...
cl_profiling_cb_blok_zsytrfsp( void *callback_arg )
{
    ...
    struct cl_blok_zsytrfsp_args_s *args     = (struct cl_blok_zsytrfsp_args_s *) callback_arg;
    ...
    cl_profiling_log_register( task->name, "blok_zsytrfsp", N, 0, 0, flops, speed );
}
#endif

#if defined(PASTIX_STARPU_PROFILING_LOG)
static void (*blok_zsytrfsp_callback)(void*) = cl_profiling_cb_blok_zsytrfsp;
#else
...
fct_blok_zsytrfsp_cost( struct starpu_task          *task,
                       ...
                       unsigned                      nimpl )
{
    struct cl_blok_zsytrfsp_args_s *args = (struct cl_blok_zsytrfsp_args_s *)(task->cl_arg);
    ...
    case STARPU_CPU_WORKER:
        coefs = &(args->sopalin_data->cpu_models->coefficients[PastixComplex64-2][PastixKernelSYTRF][0]);
        break;
    case STARPU_CUDA_WORKER:
        coefs = &(args->sopalin_data->gpu_models->coefficients[PastixComplex64-2][PastixKernelSYTRF][0]);
        break;
    default:
        assert(0);
        return 0.;
...
static struct starpu_perfmodel starpu_blok_zsytrfsp_model = {
    ...
    .arch_cost_function = fct_blok_zsytrfsp_cost,
#else
...
fct_blok_zsytrfsp_cpu( void *descr[], void *cl_arg )
{
    ...
    cpucblk_zsytrfsp1d_sytrf( args->sopalin_data->solvmtx, args->cblk, L );
}
...
starpu_task_blok_zsytrf( sopalin_data_t *sopalin_data,
                         ...
                         int             prio )
{
    struct cl_blok_zsytrfsp_args_s *cl_arg    = NULL;
    int                             need_exec = 1;
    ...
    pastix_starpu_insert_task(
        &cl_blok_zsytrfsp_cpu,
        STARPU_CL_ARGS,                 cl_arg,                sizeof( struct cl_blok_zsytrfsp_args_s ),
#if defined(PASTIX_STARPU_PROFILING)
        STARPU_CALLBACK_WITH_ARG_NFREE, blok_zsytrfsp_callback, cl_arg,
#endif
