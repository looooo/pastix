 ...
 *******************************************************************************
 *
 * @brief Submits starpu hetrfsp cblk or blok task.
 *
 *******************************************************************************
 *
 ...
 *******************************************************************************/
void
starpu_task_zhetrfsp( sopalin_data_t *sopalin_data,
                      SolverCblk     *cblk,
                      int             prio )
{
    ...
    if ( cblk->cblktype & CBLK_TASKS_2D ) {
        starpu_task_blok_zhetrf( sopalin_data, cblk, prio );
        ...
        starpu_task_cblk_zhetrfsp( sopalin_data, cblk, prio );
    }
}
 ...
 *******************************************************************************/
void
starpu_task_hetrf_zgemmsp( sopalin_data_t *sopalin_data,
                           SolverCblk     *cblk,
                           SolverBlok     *blokB,
                           SolverCblk     *fcblk,
                           int             prio )
 ...
 ******************************************************************************/
void
starpu_zhetrf_sp1dplus_rl( sopalin_data_t              *sopalin_data,
                           starpu_sparse_matrix_desc_t *desc )
{
    ...
    for ( k = 0; k < solvmtx->cblknbr; k++, cblk++ ) {
        ...
        /* Unregister the temporary workspace generated by hetrf */
 ...
 ******************************************************************************/
void
starpu_zhetrf_sp1dplus_ll( sopalin_data_t              *sopalin_data,
                           starpu_sparse_matrix_desc_t *desc )
{
    const SolverMatrix *solvmtx = sopalin_data->solvmtx;
    SolverCblk         *cblk, *fcblk;
    ...
    for ( k = 0; k < solvmtx->cblknbr; k++, fcblk++ ) {
        ...
        starpu_task_cblk_zhetrfsp( sopalin_data, fcblk, prio );
    }
 ...
 ******************************************************************************/
void
starpu_zhetrf_sp2d_rl( sopalin_data_t              *sopalin_data,
                       starpu_sparse_matrix_desc_t *desc )
{
    const SolverMatrix *solvmtx = sopalin_data->solvmtx;
    SolverCblk         *cblk, *fcblk;
    ...
    for ( k = 0; k <= solvmtx->cblkmax1d; k++, cblk++ ) {
        ...
        starpu_task_zhetrfsp( sopalin_data, cblk, prio );
        ...
        for ( m = 0; blok < lblk; blok++, m++ ) {
            ...
            starpu_task_hetrf_zgemmsp( sopalin_data, cblk, blok, fcblk,
                                       cblknbr - pastix_imin( k + m, cblk_n ) );
        }
 ...
 ******************************************************************************/
void
starpu_zhetrf_sp2d_ll( sopalin_data_t              *sopalin_data,
                       starpu_sparse_matrix_desc_t *desc )
{
    const SolverMatrix *solvmtx = sopalin_data->solvmtx;
    SolverCblk         *cblk, *fcblk;
    ...
    for ( k = 0; k < cblknbr; k++, fcblk++ ) {
        ...
        starpu_task_zhetrfsp( sopalin_data, fcblk, prio );
    }
               ...
               sopalin_data_t *sopalin_data )
{
    ...
    if ( pastix_data->iparm[IPARM_TASKS2D_LEVEL] != 0 )
    {
        if ( pastix_data->iparm[IPARM_FACTO_LOOK_SIDE] == PastixFactLeftLooking ) {
            starpu_zhetrf_sp2d_ll( sopalin_data, sdesc );
        }
        else {
            starpu_zhetrf_sp2d_rl( sopalin_data, sdesc );
        }
    }
    else
    {
        if ( pastix_data->iparm[IPARM_FACTO_LOOK_SIDE] == PastixFactLeftLooking ) {
            starpu_zhetrf_sp1dplus_ll( sopalin_data, sdesc );
        }
        else {
            starpu_zhetrf_sp1dplus_rl( sopalin_data, sdesc );
        }
    }
