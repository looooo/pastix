...
/**
 * @brief Main structure for all tasks of blok_zpxtrfsp type
 */
 ...
 */
starpu_profile_t blok_zpxtrfsp_profile = {
    .next = NULL,
    .name = "blok_zpxtrfsp"
};
 ...
 */
void blok_zpxtrfsp_profile_register( void ) __attribute__( ( constructor ) );
void
blok_zpxtrfsp_profile_register( void )
{
    profiling_register_cl( &blok_zpxtrfsp_profile );
}

#ifndef DOXYGEN_SHOULD_SKIP_THIS
#if defined(PASTIX_STARPU_PROFILING_LOG)
static void
cl_profiling_cb_blok_zpxtrfsp( void *callback_arg )
{
    ...
    struct cl_blok_zpxtrfsp_args_s *args     = (struct cl_blok_zpxtrfsp_args_s *) callback_arg;
    ...
    cl_profiling_log_register( task->name, "blok_zpxtrfsp", N, 0, 0, flops, speed );
}
#endif

#if defined(PASTIX_STARPU_PROFILING_LOG)
static void (*blok_zpxtrfsp_callback)(void*) = cl_profiling_cb_blok_zpxtrfsp;
#else
static void (*blok_zpxtrfsp_callback)(void*) = cl_profiling_callback;
#endif
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
...
fct_blok_zpxtrfsp_cost( struct starpu_task           *task,
                        struct starpu_perfmodel_arch *arch,
                        unsigned                      nimpl )
{
    struct cl_blok_zpxtrfsp_args_s *args = (struct cl_blok_zpxtrfsp_args_s *)(task->cl_arg);
    ...
    case STARPU_CPU_WORKER:
        coefs = &(args->sopalin_data->cpu_models->coefficients[PastixComplex64-2][PastixKernelPXTRF][0]);
        break;
    case STARPU_CUDA_WORKER:
        coefs = &(args->sopalin_data->gpu_models->coefficients[PastixComplex64-2][PastixKernelPXTRF][0]);
        break;
    default:
        assert(0);
        return 0.;
...
static struct starpu_perfmodel starpu_blok_zpxtrfsp_model = {
    ...
    .arch_cost_function = fct_blok_zpxtrfsp_cost,
#else
 ...
 *******************************************************************************/
static void
fct_blok_zpxtrfsp_cpu( void *descr[], void *cl_arg )
{
    ...
    cpucblk_zpxtrfsp1d_pxtrf( args->sopalin_data->solvmtx, args->cblk, L );
}
...
starpu_task_blok_zpxtrf( sopalin_data_t *sopalin_data,
                         ...
                         int             prio )
{
    struct cl_blok_zpxtrfsp_args_s *cl_arg    = NULL;
    int                             need_exec = 1;
...
#if defined(PASTIX_STARPU_PROFILING)
        cl_arg->profile_data.measures = blok_zpxtrfsp_profile.measures;
    ...
    asprintf( &task_name, "%s( %ld )",
              cl_blok_zpxtrfsp_cpu.name,
              (long)(cblk - sopalin_data->solvmtx->cblktab) );
#endif

    pastix_starpu_insert_task(
        &cl_blok_zpxtrfsp_cpu,
        STARPU_CL_ARGS,                 cl_arg,                sizeof( struct cl_blok_zpxtrfsp_args_s ),
#if defined(PASTIX_STARPU_PROFILING)
        STARPU_CALLBACK_WITH_ARG_NFREE, blok_zpxtrfsp_callback, cl_arg,
#endif
