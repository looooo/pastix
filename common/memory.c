                      ...
                      ...)
{
  ...
  void    **  memloc;   /* Pointer to pointer of current argument */
  size_t      memoff;   /* Offset value of argument               */
  void    *   blkptr;   /* Pointer to memory chunk                */

  memoff = 0;
  memloc = (void **) memptr;             /* Point to first memory argument */
  ...
  while (memloc != NULL) {               /* As long as not NULL pointer    */
    ...
    memloc  = va_arg (memlist, void **);
  }

  if ((blkptr = (void *) memAlloc (memoff)) == NULL) {
    /* If cannot allocate   */
                        ...
                        ...)
{
  ...
  if ((blkptr = (void *) memRealloc (oldptr, memoff)) == NULL)
    return (NULL);
