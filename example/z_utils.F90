...
Contains
  ...
  Subroutine get_option(driver_num, filename, nbthread, verbose, matsize)
    ...
    pastix_int_t,           Intent(out) :: nbthread
    pastix_int_t,           Intent(out) :: verbose
  ...
  Subroutine read_matrix(driver_type, filename, &
    ...
    DOUBLE COMPLEX,  Dimension(:), Allocatable, Intent(out)   :: val ! Value of each element
    DOUBLE COMPLEX,  Dimension(:), Allocatable, Intent(out)   :: rhs ! right hand side
    ...
    !      print *, mpid, ja(1)
    !      print *, mpid, "val"
    call MPI_Bcast(val,nnzero, MPI_DOUBLE_COMPLEX, 0, pastix_comm, ierr)
    !      print *, mpid, val(1)
    !      print *, mpid, "rhs"
    call MPI_Bcast(rhs,n     , MPI_DOUBLE_COMPLEX, 0, pastix_comm, ierr)
    !      print *, mpid, "type"
  ...
  Subroutine read_rsa_header(filename, nrow, ncol, nnzero, type, rhstype, ierr)
    ...
    DOUBLE COMPLEX,  Dimension(0)                           :: val ! Value of each element
    DOUBLE COMPLEX,  Dimension(0)                           :: rhs ! right hand side
    Integer                                                 :: tmp
    Integer                                                 :: nrhs
    Character(len=8)                                        :: key
    Character(len=72)                                       :: title
...
!!$    rhstype - Type of the right hand side.			       	 
  ...
  subroutine genlaplacian(n, nnzeros, ia, ja, avals, rhs, type, rhstype,success)
    ...
    DOUBLE COMPLEX,   dimension(:) , allocatable, intent(out) :: avals,rhs
    character(len=4)         , intent(out)                    :: type
    character(len=4)         , intent(out)                    :: rhstype
    integer                                                   :: success
