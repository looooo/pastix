...
int z_spm_matvec_check( int trans, const pastix_csc_t *spm );
int c_spm_matvec_check( int trans, const pastix_csc_t *spm );
int d_spm_matvec_check( int trans, const pastix_csc_t *spm );
int s_spm_matvec_check( int trans, const pastix_csc_t *spm );
    ...
    for( baseval=0; baseval<2; baseval++ )
    {
        ...
        for( mtxtype=PastixGeneral; mtxtype<=PastixHermitian; mtxtype++ )
        {
            ...
            case PastixComplex64:
                ret = z_spm_matvec_check( PastixNoTrans, &csc );
                break;

            case PastixComplex32:
                ret = c_spm_matvec_check( PastixNoTrans, &csc );
                break;

            case PastixFloat:
                ret = s_spm_matvec_check( PastixNoTrans, &csc );
                break;

            case PastixDouble:
            default:
                ret = d_spm_matvec_check( PastixNoTrans, &csc );
            }
            PRINT_RES(ret);
        }
    }
