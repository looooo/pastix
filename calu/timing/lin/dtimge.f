...
*        ALLOCATE L and IPIV
*
         CALL PLASMA_ALLOCATE_L( L, M, N )
         CALL PLASMA_ALLOCATE_IPIV( IPIV, M, N )
*
...
*
               IF( TIMSUB( 1 ) ) THEN
                  ...
                  S1 = DSECND( )
   10             CONTINUE
                  CALL PLASMA_DGETRF( M, N, A, LDA, L, IPIV, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
            IF( TIMSUB( 2 ) ) THEN
               DO 80 I = 1, NNS
                  ...
                  S1 = DSECND( )
   60             CONTINUE
                  CALL PLASMA_DGETRS( M, NRHS, A, LDA,
     $                                L, IPIV, B, LDB, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
         CALL PLASMA_FREE_L( L, INFO )
         CALL PLASMA_FREE_IPIV( IPIV, INFO )
*
  100 CONTINUE
*
...
*
  ...
  110       CONTINUE
     ...
     $                   LDR1, LDR2, NOUT )
            CALL DRAWER( 'DGETRF', 'ps', 'NB', 'N', 'Mflop/s', NNB,
     &                   NBVAL, NM, MVAL, NLDA,
     &                   RESLTS, LDR1, LDR2 ) 
         ELSE IF( ISUB.EQ.2 ) THEN
            CALL DPRTBL( 'NRHS', 'N', NNS, NSVAL, NM, MVAL, NLDA,
     $                   RESLTS( 1, 1, 1, 2 ), LDR1, LDR2, NOUT )
            CALL DRAWER( 'DGETRS', 'ps', 'NRHS', 'N', 'Mflop/s', NNS,
     &                   NSVAL, NM, MVAL, NLDA,
     &                   RESLTS( 1, 1, 1, 2 ), LDR1, LDR2 ) 
