...
*     .. Local Arrays ..
      ...
      INTEGER            PLASMA_SIDES( 2 ), PLASMA_TRANSS( 2 )
...
*     .. Data statements ..
      ...
      DATA               PLASMA_SIDES / PLASMALEFT, PLASMARIGHT / 
      DATA               PLASMA_TRANSS / PLASMANOTRANS, PLASMATRANS /
      DATA               ISEED / 0, 0, 0, 1 /
*     ..
...
*                  CALL DGELQF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  CALL PLASMA_DGELQF( M, N, A, LDA, T, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*     $                         INFO )
                  CALL PLASMA_DORGLQ( MINMN, N, MINMN, A, LDA, T, B,
     $                         LDA, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*               DO 160 ISIDE = 1, 2
               DO 160 ISIDE = 1, 1
*                 Only PLASMALEFT is supported !                   
                  SIDE = SIDES( ISIDE )
                  PLASMA_SIDE = PLASMA_SIDES( ISIDE )
*
*
...
*     $                                  TAU, B, LDA, WORK, LW, INFO )
                           CALL PLASMA_DORMLQ( PLASMA_SIDE, 
     $                                         PLASMA_TRANS, M1, N1, K1,
     $                                         A, LDA, T, B, LDA, INFO )
                           S2 = DSECND( )
                           TIME = S2 - S1
                           IC = IC + 1
