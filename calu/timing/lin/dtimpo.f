...
*     .. Data statements ..
      DATA               UPLOS / 'U', 'L' /
      DATA               PLASMA_UPLOS / PLASMAUPPER, PLASMALOWER /
...
*
      DO 110 IUPLO = 1, 2
         UPLO = UPLOS( IUPLO )
         PLASMA_UPLO = PLASMA_UPLOS( IUPLO )
*
...
*
                  IF( TIMSUB( 1 ) ) THEN
                     ...
                     S1 = DSECND( )
   10                CONTINUE
                     CALL PLASMA_DPOTRF( PLASMA_UPLO, N, A, LDA, INFO )
                     S2 = DSECND( )
                     TIME = S2 - S1
                     IC = IC + 1
...
*
               IF( TIMSUB( 2 ) ) THEN
                  DO 80 I = 1, NNS
                     ...
                     S1 = DSECND( )
   60                CONTINUE
                     CALL PLASMA_DPOTRS( PLASMA_UPLO, N, NRHS, A, LDA, 
     $                                   B, LDB, INFO )
                     S2 = DSECND( )
                     TIME = S2 - S1
                     IC = IC + 1
...
*
  ...
  120       CONTINUE
     ...
     $                      RESLTS( 1, 1, I3, 1 ), LDR1, LDR2, NOUT )
               CALL DRAWER( 'DPOTRF', 'ps', 'NB', 'N', 'Mflop/s', NNB,
     &                      NBVAL, NN, NVAL, NLDA,
     &                      RESLTS( 1, 1, I3, 1 ), LDR1, LDR2 ) 
            ELSE IF( ISUB.EQ.2 ) THEN
               CALL DPRTBL( 'NRHS', 'N', NNS, NSVAL, NN, NVAL, NLDA,
     $                      RESLTS( 1, 1, I3, 2 ), LDR1, LDR2, NOUT )
               CALL DRAWER( 'DPOTRS', 'ps', 'NRHS', 'N', 'Mflop/s', NNS,
     &                      NSVAL, NN, NVAL, NLDA,
     &                      RESLTS( 1, 1, I3, 2 ), LDR1, LDR2 ) 
